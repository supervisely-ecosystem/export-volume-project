...
class VolumeObjectApi(ObjectApi):
    def append_bulk(
        self, dataset_id, objects: VolumeObjectCollection, key_id_map: KeyIdMap = None
    ):
    ...
    def _get_interpolation(self, volume_id, object_id):
        return self._api.post(
            ...
            {VOLUME_ID: volume_id, ApiField.OBJECT_ID: object_id},
        )
    ...
    def get_interpolation(self, volume_id, objects: VolumeObjectCollection, key_id_map):
        ...
        for object in objects:
            ...
            try:
                ...
                logger.warn(
                    f"Error while building interpolation on server for object {object_id} : {e}"
                )
                results.append(None)
        return results
