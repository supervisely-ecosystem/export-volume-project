...
def convert_stl_to_nrrd(nrrd_path, stl_path, output_file_path):
    ...
    offsets = [math.ceil(center_v - shape_v / 2) for center_v, shape_v in zip(center, mask.shape)]

    padded_mask[
       offsets[0]:mask.shape[0] + offsets[0],
       offsets[1]:mask.shape[1] + offsets[1],
       offsets[2]:mask.shape[2] + offsets[2],
    ] = mask
